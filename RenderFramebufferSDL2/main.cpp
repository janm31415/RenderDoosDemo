...
int _main(int argc, char** argv)
  ...
  SDL_Window* window = SDL_CreateWindow("RenderFramebufferSDL2",
  ...
  std::string script = std::string(R"(void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  uint32_t framebuffer_id = engine.add_frame_buffer(800, 450, false);
  ...
  while (!quit)
    ...
    simple_mat.set_texture(engine.get_frame_buffer(framebuffer_id)->texture_handle, TEX_WRAP_REPEAT | TEX_FILTER_NEAREST);
