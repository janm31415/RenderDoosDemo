...
{
public:
  canvas(int x, int y, int w, int h, const char* t) : Fl_Metal_Window(x, y, w, h, t), _st_material(nullptr), _smpl_material(nullptr)
  {
#else
    class canvas : public Fl_Gl_Window
    {
    public:
      canvas(int x, int y, int w, int h, const char* t) : Fl_Gl_Window(x, y, w, h, t), _st_material(nullptr), _smpl_material(nullptr)
      {
      ...
      virtual void hide()
      {
        _engine.destroy();
        delete _st_material;
        _st_material = nullptr;
        delete _smpl_material;
        _smpl_material = nullptr;
#if defined(RENDERDOOS_METAL)
...
#endif
          _st_material = new RenderDoos::shadertoy_material();
          _smpl_material = new RenderDoos::simple_material();
    ...
    fragColor = vec4(col,1.0);
...
#endif
          _st_material->set_script(script);
          _st_material->compile(&_engine);
          _smpl_material->compile(&_engine);
          _geometry_id = _engine.add_geometry(VERTEX_STANDARD);
          _framebuffer_id = _engine.add_frame_buffer(800, 450, false);
        ...
        virtual void _draw(MTL::Device* device, CA::MetalDrawable* drawable)
        {
          ...
          virtual void draw()
          {
            ...
            _smpl_material->set_texture(_engine.get_frame_buffer(_framebuffer_id)->texture_handle, TEX_WRAP_REPEAT | TEX_FILTER_NEAREST);
            _smpl_material->bind(&_engine);
            _engine.geometry_draw(_geometry_id);
            _engine.renderpass_end();
        ...
        private:
          ...
          int32_t _geometry_id, _framebuffer_id;
          RenderDoos::shadertoy_material* _st_material;
          RenderDoos::simple_material* _smpl_material;
        ...
        render_window* new_view(int w = 1600, int h = 900)
        {
          int x = 100;
          int y = 100;
#ifdef _WIN32
...
#endif
          render_window* window = new render_window(x, y, w, h, "RenderFramebuffer");
